#!/usr/bin/env python3
"""
LeRobot GROOT N1.5 Robot Inference for SO-101.

This script runs a finetuned GROOT N1.5 policy on the real SO-101 robot arm
using LeRobot's infrastructure.

GROOT N1.5 Features:
- NVIDIA's foundation model for generalized robot manipulation
- Cross-embodiment capabilities with embodiment-specific MLPs
- Eagle2.5-HG vision-language backbone
- Flow matching diffusion transformer for action prediction
- Language/task description conditioning

Architecture:
    Input: Images (head, wrist) + Language + Proprioception
           ↓
    Eagle2.5-HG Vision-Language Backbone
           ↓
    VLLN Layer (Vision-Language LayerNorm)
           ↓
    State Encoder (CategorySpecificMLP)
           ↓
    DiT Action Head (16 layers, flow matching)
           ↓
    Action Decoder (Multi-embodiment)
           ↓
    Output: Action chunk [B, horizon, action_dim]

Execution Flow:
1. Load hardware config from YAML
2. Initialize robot connection (serial port)
3. Initialize cameras (head + left_wrist)
4. Load GrootPolicy with checkpoint and preprocessors
5. Main loop:
...
Usage:
    python infer_groot_so101_n15.py --checkpoint outputs/groot_so101_n15/checkpoints/last
    python infer_groot_so101_n15.py -c outputs/groot_so101_n15 --task "pick up the block" --dry-run

Reference:
    - https://huggingface.co/docs/lerobot/groot
    - lerobot/policies/groot/modeling_groot.py
"""
...
# ============================================================================
# Model
DEFAULT_CHECKPOINT = "outputs/groot_so101_n15/checkpoints/last"
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

# Task description (language prompt for GROOT)
DEFAULT_TASK = "Pick up the block and place it on the plate"

# Inference Settings
ACTION_INTERVAL = 0.033   # 30Hz execution rate (1/30 seconds)

# GROOT specific settings
CHUNK_SIZE = 50           # GROOT default
N_ACTION_STEPS = 50       # Execute full chunk by default

# Hardware Config (external file)
HARDWARE_CONFIG = "jdocs/scripts/so101_hardware.yaml"

# Dataset (for loading stats)
...
def format_observation(
    ...
    """Format observation for GROOT policy input.

    GROOT expects (via processor pipeline):
    ...
    The processor will:
        1. Resize images to 224x224 (GROOT's expected size)
        2. Normalize state with MEAN_STD
        3. Tokenize language with Eagle processor
        4. Create embodiment_id tensor
    """
    observation = {}
    ...
    # GROOT processor will resize to 224x224
    ...
    # Task description for GROOT (language conditioning)
...
def run_inference_loop(
    ...
    if record_images:
        record_dir = PROJECT_ROOT / "jdocs" / "eval_images" / f"inference_groot_{int(start_time)}"
    ...
    while running and (time.time() - start_time) < max_duration:
        ...
        # Run preprocessing (normalization, image resize, tokenization)
...
def main():
    parser = argparse.ArgumentParser(
        description="LeRobot GROOT N1.5 robot inference for SO-101"
    )
    parser.add_argument(
        "--checkpoint", "-c",
        type=str,
    ...
    log_file = LOG_DIR / f"inference_groot_n15_{timestamp}.log"
    ...
    logger.info("=" * 70)
    logger.info("LeRobot GROOT N1.5 Robot Inference for SO-101")
    logger.info("=" * 70)
    ...
    if not checkpoint_path.exists():
        ...
        if not args.checkpoint.startswith("nvidia/") and not args.checkpoint.startswith("lerobot/"):
    ...
    try:
        ...
        # Load GROOT policy
        logger.info("\nLoading GROOT N1.5 policy...")
        logger.info("  (This may take a moment to download Eagle backbone...)")

        from lerobot.policies.groot.modeling_groot import GrootPolicy
        from lerobot.policies.factory import make_pre_post_processors

        policy = GrootPolicy.from_pretrained(str(checkpoint_path))
        policy.eval()
        policy.to(args.device)

        logger.info(f"  Policy loaded on {args.device}")
        logger.info(f"  Base model: {policy.config.base_model_path}")
        logger.info(f"  Embodiment tag: {policy.config.embodiment_tag}")

#!/usr/bin/env python3
"""
Pi0.5 Robot Inference for SO-101.

This script runs a finetuned Pi0.5 (Physical Intelligence's VLA model) policy
on the real SO-101 robot arm using LeRobot's infrastructure.

Pi0.5 features:
- PaliGemma (vision-language) + Gemma Expert (action) architecture
...
4. Load PI05Policy with checkpoint and preprocessors
5. Main loop:
...
Usage:
    python infer_pi05_so101.py --checkpoint outputs/pi05_pickplace_*/checkpoints/003000/pretrained_model
    python infer_pi05_so101.py -c outputs/pi05_pickplace --task "pick up the block" --dry-run
...
def main():
    parser = argparse.ArgumentParser(
        description="Pi0.5 robot inference for SO-101"
    )
    parser.add_argument(
        "--checkpoint", "-c",
        type=str,
    ...
    log_file = LOG_DIR / f"inference_pi05_{timestamp}.log"
    ...
    logger.info("=" * 70)
    logger.info("Pi0.5 Robot Inference for SO-101")
    logger.info("=" * 70)
    ...
    try:
        ...
        logger.info(f"  Num inference steps: {policy.config.num_inference_steps}")

#!/usr/bin/env python3
"""
ACT Robot Inference for SO-101.

This script runs a finetuned ACT (Action Chunking with Transformers) policy
...
4. Load ACTPolicy with checkpoint and preprocessors
5. Main loop:
   a. Capture images from both cameras
   b. Read robot state (6 DOF)
   c. Format observation dict with batch dimensions
   d. Run policy.select_action(observation)
   e. Execute action at 30Hz
   f. Repeat until duration exceeded or user stops

Observation Format (ACT/LeRobot expects):
    observation = {
        "observation.state": torch.Tensor,          # (B=1, D=6)
        "observation.images.head": torch.Tensor,    # (B=1, C=3, H=480, W=640)
        "observation.images.left_wrist": torch.Tensor,  # (B=1, C=3, H=480, W=640)
    }

Usage:
    python infer_act_so101.py --checkpoint outputs/act_pickplace/checkpoints/100000/pretrained_model
    python infer_act_so101.py -c outputs/act_pickplace --dry-run

Reference: examples/tutorial/act/act_using_example.py
"""
...
# ============================================================================
# Model
DEFAULT_CHECKPOINT = "outputs/act_pickplace/checkpoints/100000/pretrained_model"
...
def format_observation(
    ...
    """Format observation for ACT policy input.
...
def run_inference_loop(
    ...
    while running and (time.time() - start_time) < max_duration:
        ...
        # Format observation
        observation = format_observation(images, state, device)

        # Run preprocessing (normalization)
...
def main():
    parser = argparse.ArgumentParser(
        description="ACT robot inference for SO-101"
    )
    parser.add_argument(
        "--checkpoint", "-c",
        type=str,
    ...
    logger.info("=" * 70)
    logger.info("ACT Robot Inference for SO-101")
    logger.info("=" * 70)

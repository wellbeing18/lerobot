#!/usr/bin/env python3
"""
xVLA Robot Inference for SO-101.

This script runs a finetuned xVLA (Cross-Embodiment Vision-Language-Action)
policy on the real SO-101 robot arm using LeRobot's infrastructure.

xVLA differs from SmolVLA:
- 0.9B parameters (larger model)
- Soft prompts for multi-embodiment learning
- Domain IDs for different robot/camera configurations
- Action modes for different action space handling
- Florence2 VLM backbone with ImageNet normalization

Execution Flow:
1. Load hardware config from YAML
2. Initialize robot connection (serial port)
3. Initialize cameras (head + left_wrist)
4. Load XVLAPolicy with checkpoint and preprocessors
5. Main loop:
   a. Capture images from both cameras
   b. Read robot state (6 DOF)
   c. Format observation dict with task description and domain_id
   d. Apply preprocessing (ImageNet normalization, tokenization)
   e. Run policy.select_action(observation)
   f. Apply postprocessing (action mode handling)
   g. Execute action at 30Hz
   h. Repeat until duration exceeded or user stops

Usage:
    # Basic inference
    python infer_xvla_so101.py \\
        --checkpoint outputs/xvla_pickplace_*/checkpoints/020000/pretrained_model
    ...
    python infer_xvla_so101.py -c outputs/xvla_pickplace --dry-run

References:
    - https://huggingface.co/docs/lerobot/xvla
    - examples/xvla/ (if available)
"""
...
# ============================================================================
# Model
DEFAULT_CHECKPOINT = "outputs/xvla_pickplace/checkpoints/020000/pretrained_model"
...
DEFAULT_DOMAIN_ID = 20  # Custom domain for SO-101
...
# ImageNet normalization constants (required for xVLA)
...
def format_observation(
    ...
    """Format observation for xVLA policy input.

    xVLA expects:
        observation.state: (B=1, D=6) float32 tensor
        observation.images.camera1: (B=1, C=3, H=480, W=640) float32 tensor [0, 1]
        observation.images.camera2: (B=1, C=3, H=480, W=640) float32 tensor [0, 1]
        task: str (language description)
        domain_id: int (robot/camera configuration identifier)

    Note: xVLA preprocessing will apply ImageNet normalization automatically
    via the preprocessor pipeline.
    """
    observation = {}
    ...
    # Map camera names: head -> camera1, left_wrist -> camera2
    key_mapping = {
        "head": "camera1",
        "left_wrist": "camera2"
    }
    ...
    for name, frame in images.items():
        # Map specific camera names to xVLA expected names
        policy_key_name = key_mapping.get(name, name)

        # frame is (H, W, C) uint8 RGB, convert to (B=1, C, H, W) float32 [0, 1]
    ...
    # Task description for xVLA (language conditioning)
    observation["task"] = task

    # Domain ID for xVLA (identifies robot/camera configuration)
    # This will be converted to tensor by the preprocessor
...
def run_inference_loop(
    ...
    while running and (time.time() - start_time) < max_duration:
        ...
        # Format observation with task description and domain_id
        observation = format_observation(images, state, task, domain_id, device)

        # Run preprocessing (normalization, tokenization, ImageNet norm, domain_id injection)
        ...
        # Postprocess (unnormalize, action mode handling)
        ...
        if step_count % 30 == 0:
            ...
            # Log action
            action_str = ", ".join([f"{joint_names[i]}={action[i]:.1f}" for i in range(min(len(action), len(joint_names)))])
            logger.info(f"  Action: [{action_str}]")
            # Log action delta (action - state)
            delta = action[:len(state)] - state
    ...
    if action_history:
        ...
        if state_arr is not None and len(state_arr) > 0:
            ...
            logger.info(f"\nGripper Analysis:")
            gripper_actions = action_arr[:, -1] if action_arr.shape[1] > 5 else action_arr[:, 5]
...
def main():
    parser = argparse.ArgumentParser(
        description="xVLA robot inference for SO-101",
    ...
    parser.add_argument(
        ...
        help=f"Domain ID for robot/camera configuration (default: {DEFAULT_DOMAIN_ID})"
    )
    parser.add_argument(
        "--duration",
        type=float,
    ...
    log_file = LOG_DIR / f"inference_xvla_{timestamp}.log"
    ...
    logger.info("=" * 70)
    logger.info("xVLA Robot Inference for SO-101")
    logger.info("=" * 70)
    ...
    try:
        ...
        # xVLA requires special preprocessing: ImageNet normalization, domain_id injection
        preprocessor, postprocessor = make_pre_post_processors(
            ...
            preprocessor_overrides={
                ...
                # Domain ID will be added from observation dict
            },
        )

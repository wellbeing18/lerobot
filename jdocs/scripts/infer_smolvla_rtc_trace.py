#!/usr/bin/env python3
"""
SmolVLA Real-Time Chunking (RTC) Inference with Trace Recording for SO-101.

This script extends infer_smolvla_rtc.py with comprehensive tracing to investigate
...
Usage:
    python infer_smolvla_rtc_trace.py \\
        --checkpoint outputs/smolvla_pickplace/checkpoints/020000/pretrained_model \\
        --task "pick up the block and place it on the plate" \\
        --duration 30

References:
    - jdocs/scripts/infer_smolvla_rtc.py (base script)
...
def setup_logging(log_file: Path = None) -> logging.Logger:
    ...
    logger = logging.getLogger("smolvla_rtc_trace")
    ...
    try:
        ...
        while not shutdown_event.is_set():
            if action_queue.qsize() <= action_queue_threshold:
                ...
                inference_latency = latency_tracker.max() or 0.5
                ...
                chunk_start_idx = action_index_before

                # Save images on inference steps (use hardware camera names)
                if save_images:
                    for name, frame in images.items():
        ...
        while not shutdown_event.is_set():
            ...
            if action is not None:
                ...
                trace_entry = TraceEntry(
                    ...
                    action_idx_in_chunk=chunk_action_idx,
...
def main():
    parser = argparse.ArgumentParser(
        description="SmolVLA RTC inference with tracing for SO-101",
    ...
    trace_dir = Path(args.trace_output_dir) / f"trace_smolvla_{timestamp}"
    ...
    logger.info("=" * 70)
    logger.info("SmolVLA RTC Inference with Tracing for SO-101")
    logger.info("=" * 70)
    ...
    try:
        ...
        get_thread = Thread(
            ...
            args=(policy, preprocessor, postprocessor, cameras, robot,
                  ...
                  not args.no_rtc, trace_start, images_dir, not args.no_save_images),
            daemon=True,
            name="GetActions"
        )
        get_thread.start()

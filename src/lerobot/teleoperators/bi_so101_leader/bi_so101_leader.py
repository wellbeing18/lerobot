...
class BiSO101Leader(Teleoperator):
    ...
    This class wraps two SO101Leader instances for coordinated bimanual teleoperation.
    """
    ...
    def __init__(self, config: BiSO101LeaderConfig):
        ...
        # Use explicit arm IDs if provided, otherwise derive from parent ID
        left_id = config.left_arm_id or (f"{config.id}_left" if config.id else None)
        right_id = config.right_arm_id or (f"{config.id}_right" if config.id else None)
